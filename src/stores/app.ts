  ...
  actions: {
    ...
    async fetchCharacterInfo(id: string) {
      try {
        ...
        this.characterInfo?.episode.forEach((episode: string) => {
          this.episodesUrl.push(
            episode.split("/")[episode.split("/").length - 1],
          );
        });
        ...
        this.locationId =
          this.characterInfo?.location.url.split("/")[
            this.characterInfo?.location.url.split("/").length - 1
          ];
      } catch (error) {
